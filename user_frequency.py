...
calculated_users = os.listdir(os.path.abspath(os.path.join(USER_FREQUENCY_DIR)))
calculated_users = list(map(lambda extracted: extracted.split('.')[0], calculated_users))

for index, user_rated in enumerate(users_rated):
    if (user_rated not in calculated_users):
